...
n5_path=/besu-25.3.0/QBFT-Network/Node-5/cfg_n5.toml

echo "# NODE 4 Config File" > $n5_path
...
echo '# graphql' >> $n5_path
echo 'graphql-http-enabled=true' >> $n5_path
echo 'graphql-http-host="0.0.0.0"' >> $n5_path
echo 'graphql-http-port=9549' >> $n5_path
echo 'graphql-http-cors-origins=["all"]' >> $n5_path
echo -e "\n" >> $n5_path

echo "# Bootnodes" >> $n5_path
echo 'bootnodes=["'$enode_address'"]' >> $n5_path

cd /besu-25.3.0/QBFT-Network/Node-5
